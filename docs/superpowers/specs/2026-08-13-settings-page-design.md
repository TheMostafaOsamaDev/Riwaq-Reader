...
**new settings** (Reading, Behavior, Downloads, Data/About). No new dependencies.

New: `src/components/SettingsPage.tsx` (the page), `src/components/SettingsSection.tsx`
+ shared field primitives (extracted), `src/hooks/useWakeLock.ts`. Touches
`src/App.tsx` (top-level view + navigation state, wake lock, startup view, reduce-motion
override wiring), `src/types/reader.ts` (`Tweaks` new fields), `src/hooks/useTweaks.ts`
(defaults + bulk apply + migration), `src/components/Library.tsx`
(rewire `onOpenSettings` to navigate; remove sheet mount; confirm-delete honoring),
`src/panels/SettingsPanel.tsx` (new reading toggles + "Open full settings" link),
`src/store/downloadQueue.ts` (runtime concurrency + Wi-Fi gate), `src/styles/motion.ts`
(reduce-motion override), `src/i18n/en.ts` + `src/i18n/ar.ts` (new keys, both catalogs).
...
`src/i18n/ar.ts` (section titles, each control label, the Wi-Fi/keep-awake best-effort
hints, Data action labels + confirm copy, About strings). The build gate enforces
parity. Arabic typography conventions already handled by the shared primitives
(no uppercase/tracking on `ar`) carry over.
