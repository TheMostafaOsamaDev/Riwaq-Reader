...
const TREE: { key: LibraryTab; label: string }[] = [
    ...
    >
      ...
      <button
        ...
        style={{
          ...
          cursor: "pointer", font: "inherit", color: theme.muted, textAlign: "left", transition: TRANSITION,
        }}
        onMouseEnter={(e) => (e.currentTarget.style.background = theme.hover)}
        onMouseLeave={(e) => (e.currentTarget.style.background = theme.bg)}
      >
        <Icon name="search" size={16} />
        <span style={{ flex: 1, fontSize: 13 }}>Search library</span>
      ...
      <div style={{ flex: 1, minHeight: 0, overflowY: "auto", overflowX: "hidden" }}>
        ...
        <nav style={{ display: "flex", flexDirection: "column", gap: 5, padding: "0 4px" }}>
          ...
          <div>
            <CollapsibleRow theme={theme} dark={dark} icon="grid" label="Library" active={tab === "all" && !shelvesActive} open={openLib} onActivate={() => setTab("all")} setOpen={setOpenLib} />
            <Collapse open={openLib}>
              <Tree theme={theme}>
                {TREE.map((t) => (
                  <TreeButton key={t.key} theme={theme} label={t.label} active={tab === t.key} onClick={() => setTab(t.key)} />
                ))}
              </Tree>
            </Collapse>
          </div>

          <NavRow theme={theme} icon="globe" label="Store" active={tab === "store" && !shelvesActive} onClick={() => setTab("store")} />

          {/* Shelves (collapsible) — row + tree grouped so the nav gap stays uniform */}
          <div>
            <CollapsibleRow theme={theme} dark={dark} icon="layers" label="Shelves" active={shelvesActive} open={openShelves} onActivate={onOpenShelves} setOpen={setOpenShelves} />
            <Collapse open={openShelves}>
              <Tree theme={theme}>
                ...
                <button
                  onClick={onNewShelf}
                  style={{ display: "flex", alignItems: "center", gap: 8, width: "100%", textAlign: "left", border: 0, background: "transparent", color: theme.muted, font: "inherit", fontSize: 13, fontWeight: 500, padding: "8px 12px", borderRadius: 9, cursor: "pointer", transition: TRANSITION }}
                  onMouseEnter={(e) => (e.currentTarget.style.background = theme.hover)}
          ...
          <button
            ...
            onMouseLeave={(e) => (e.currentTarget.style.background = "transparent")}
          >
            <span style={{ position: "absolute", left: 0, top: 0, bottom: 0, width: dl.active ? `${Math.max(6, Math.round(dl.progress * 100))}%` : 0, background: goldSoft, borderRadius: 10, transition: "width .35s ease", zIndex: 0 }} />
            <span style={{ position: "relative", zIndex: 1, color: theme.muted, display: "flex" }}><Icon name="download" size={18} /></span>
            <span style={{ position: "relative", zIndex: 1 }}>Downloads</span>
            {dl.active ? (
              <span style={{ position: "relative", zIndex: 1, marginLeft: "auto", fontSize: 11.5, fontWeight: 600, color: theme.ink, fontVariantNumeric: "tabular-nums" }}>{Math.round(dl.progress * 100)}%</span>
            ) : dl.count > 0 ? (
              <span style={{ position: "relative", zIndex: 1, marginLeft: "auto", minWidth: 20, height: 20, padding: "0 6px", borderRadius: 10, background: goldSoft, color: gold, fontSize: 11, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center" }}>{dl.count}</span>
            ) : null}
          </button>

          <NavRow theme={theme} icon="settings" label="Settings" active={false} onClick={onOpenSettings} />
        </nav>
      </div>
        ...
        <div ref={importRef} style={{ position: "relative" }}>
          <div style={{ display: "flex" }}>
            <button
              ...
              style={{
                ...
                borderRight: `1px solid ${dark ? "rgba(0,0,0,0.25)" : "rgba(255,255,255,0.18)"}`, borderRadius: "11px 0 0 11px",
            ...
            <button
              ...
              style={{ display: "flex", alignItems: "center", justifyContent: "center", width: 38, height: 40, border: 0, borderRadius: "0 11px 11px 0", background: theme.ink, color: theme.paper, cursor: "pointer", transition: TRANSITION }}
          ...
          {menuOpen && (
            <div style={{ position: "absolute", left: 0, right: 0, bottom: "calc(100% + 6px)", background: theme.paper, border: `1px solid ${theme.rule}`, borderRadius: 12, boxShadow: "0 16px 36px rgba(0,0,0,0.18)", padding: 6, zIndex: 20 }}>
              <MenuItem theme={theme} icon="folder" label="Folder of EPUBs" onClick={() => { setMenuOpen(false); onImportFolder(); }} />
              <MenuItem theme={theme} icon="doc" label="Word document" tag=".docx" onClick={() => { setMenuOpen(false); onImportDocx(); }} />
            </div>
          )}
        </div>
      </div>
...
function Tree({ theme, children }: { theme: Theme; children: ReactNode }) {
  return (
    <div style={{ position: "relative", margin: "4px 0 0 22px", paddingLeft: 14 }}>
      <span style={{ position: "absolute", left: 0, top: 2, bottom: 14, width: 1, background: theme.rule }} />
      {children}
    </div>
  );
}
...
function TreeButton({ theme, label, active, onClick }: { theme: Theme; label: string; active: boolean; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      style={{
        position: "relative", display: "block", width: "100%", textAlign: "left", border: 0,
    ...
    >
      <span style={{ position: "absolute", left: -14, top: "50%", width: 10, height: 1, background: theme.rule }} />
      {label}
    </button>
  );
}
...
function CollapsibleRow({
  theme, dark, icon, label, active, open, onActivate, setOpen,
}: {
  theme: Theme; dark: boolean; icon: IconProps["name"]; label: string; active: boolean; open: boolean;
  onActivate: () => void; setOpen: (v: boolean) => void;
}) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "stretch", borderRadius: 10, overflow: "hidden", transition: TRANSITION }}>
      <button
        ...
        title={`Double-click to ${open ? "collapse" : "expand"}`}
        style={{
          ...
          cursor: "pointer", textAlign: "left", userSelect: "none", WebkitUserSelect: "none", transition: TRANSITION,
        }}
      ...
      <button
        ...
        style={{
          ...
          borderLeft: `1px solid ${divider}`, background: sectionBg, color: iconColor, cursor: "pointer", transition: TRANSITION,
        }}
        onMouseEnter={(e) => { if (active) e.currentTarget.style.opacity = "0.9"; else e.currentTarget.style.background = theme.hover; }}
        onMouseLeave={(e) => { if (active) e.currentTarget.style.opacity = "1"; else e.currentTarget.style.background = "transparent"; }}
      >
        <Icon name="chevronR" size={15} style={{ transform: open ? "rotate(90deg)" : "none", transition: "transform 180ms ease" }} />
      </button>
    </div>
  );
}
  ...
  return (
    <button
      ...
      style={{
        ...
        fontFamily: "inherit", cursor: "pointer", textAlign: "left", width: "100%", transition: TRANSITION,
      }}
      onMouseEnter={(e) => { if (!active) e.currentTarget.style.background = theme.hover; }}
      onMouseLeave={(e) => { if (!active) e.currentTarget.style.background = "transparent"; }}
    >
...
function MenuItem({ theme, icon, label, tag, onClick }: { theme: Theme; icon: IconProps["name"]; label: string; tag?: string; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      style={{ display: "flex", alignItems: "center", gap: 11, width: "100%", border: 0, background: "transparent", color: theme.ink, font: "inherit", fontWeight: 500, fontSize: 13, padding: 10, borderRadius: 8, cursor: "pointer", textAlign: "left", transition: TRANSITION }}
      onMouseEnter={(e) => (e.currentTarget.style.background = theme.hover)}
      onMouseLeave={(e) => (e.currentTarget.style.background = "transparent")}
    >
      <span style={{ color: theme.muted, display: "flex" }}><Icon name={icon} size={17} /></span>
      {label}
      {tag && <span style={{ marginLeft: "auto", fontSize: 10.5, color: theme.muted }}>{tag}</span>}
    </button>
  );
}
