...
describe("searchView", () => {
  it("shows skeletons while the first page loads", () => {
    expect(searchView({ loading: true, error: null, result: null })).toBe(
      "skeletons",
    );
  });

  it("shows skeletons even when stale results are still held", () => {
    expect(searchView({ loading: true, error: "boom", result: page(3) })).toBe(
      "skeletons",
    );
  });

  it("shows the error box when the first page failed and nothing is loaded", () => {
    expect(searchView({ loading: false, error: "boom", result: null })).toBe(
      "error",
    );
  });
  ...
  it("keeps the grid when an error arrives but results are already on screen", () => {
    ...
    expect(searchView({ loading: false, error: "boom", result: page(3) })).toBe(
      "grid",
    );
  });

  it("shows the empty state for a successful search with no matches", () => {
    expect(searchView({ loading: false, error: null, result: page(0) })).toBe(
      "empty",
    );
  });

  it("shows the grid for a normal populated result", () => {
    expect(searchView({ loading: false, error: null, result: page(5) })).toBe(
      "grid",
    );
  });
});
